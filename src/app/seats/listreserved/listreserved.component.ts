...
@Component({
  selector: 'app-listreserved',
  templateUrl: './listreserved.component.html',
  styleUrls: ['./listreserved.component.scss']
})
export class ListreservedComponent implements OnInit {
  ...
  constructor(
    ...
    this.getseatstotal(this.status);
  } 
  ...
  getseatstotal(status:string) {    
    this.seatsService.listSeatsTotal(status)
      .subscribe(data => {
        if (!data.error) {
  ...
  edit(seats) {
    ...
    this.router.navigate(['/seats/addseat'], navigationExtras);
  }
